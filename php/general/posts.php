...
class posts
{
	...
	const post_type_blogarticle = 12;
	
	const feed_style_classic  	= 1;
	const feed_style_garden     = 2;
	
	var $last_create_post_id = 0;
	...
	function __construct()
	{
		...
		$this->set_mode_filters(basics::setting_get(1, "filter_view"), 0);
	}
	...
	{
		return $this->create_post_geo($touser, $ptype, $tcontent, $tcontent2, $tcontent3, $addtoblog, $visibility, 0);
	}
	
	function create_blog_post($data)
	{
		return $this->create_post(0, self::post_type_blogarticle, $data, 0, 0, 1, 1);
	}
	
	/*
	 * geo details should either be zero or an array with
	 * [0] = location display name
	 * [1] = lg
	 * [2] = lt
	 * [3] = extra *optional
	 */
	function create_post_geo($touser, $ptype, $tcontent, $tcontent2, $tcontent3, $addtoblog, $visibility, $geodetails)
	{
		...
		case self::post_type_blogarticle:   		
			break;
		...
		if($geodetails)
		{
			$loc_lt    = $geodetails[2];
			$loc_lg    = $geodetails[1];
			$loc_name  = basics::escapestr($geodetails[0]);
			$loc_used  = 1;
		}
		
		basics::query("INSERT INTO posts_$touser  (uid, ptype, opid, utype, visibility, reports, rating1, rating2, rating3, rating4, wtime, ltime, mtime, rtime, type, ctext, dtext, div1, div2, tagcid, markcid, talkcid, repostscid, repostcount, remotehost, privacymode, filtergroup, loc_lt, loc_lg, loc_name, loc_used)".
						                  "values ($writeruid, 1, 0, 1, $visibility, 0, 0, 0, 0, 0, UTC_TIMESTAMP(), UTC_TIMESTAMP(), UTC_TIMESTAMP(), UTC_TIMESTAMP(), $posttype, '$ctext', '$dtext', $div1, $div2, 0, 0, 0, 0, 0, 0, 0, 0, $loc_lt, $loc_lg, '$loc_name', $loc_used)");
		...
		$this->last_create_post_id = $finalpid;
	...
	function get_last_create_post_id()
	{
		return $this->last_create_post_id;
	}
	...
	function get_profile_post_set($uid, $psince, $pmnumber, $psincemode, $psinceval)
	{
		...
		if(!$searchq)
		{
			...
			$searchcondition1 = "AND ctext LIKE '%%$searchq%%' ";
		}
		
		if(!$psinceval)
		{
			$resposts = basics::query("SELECT * FROM posts_$uid WHERE visibility=1 $searchcondition1 ORDER BY id DESC LIMIT $pmnumber"); 
		}else{
			if($psincemode == 0)
				$resposts = basics::query("SELECT * FROM posts_$uid WHERE id<$psinceval AND visibility=1 $searchcondition1 ORDER BY id DESC LIMIT $pmnumber"); 
		}
	...
	/*
	 * binary string for switches. if it's only one zero, turn off filters (show everything)
	 */
	function set_mode_filters($filterstr, $savesettings)
	{
	
		if($savesettings)
			basics::setting_set(1, "filter_view", $filterstr);
	
		if($filterstr == "0")
		{
			$this->filtercondition = "";
		}else{
			/* text, photos, links, music, videos, blog articles */
			
			$strset = str_split($filterstr);
			$c = count($strset);
			...
			for($i=0; $i<$c; $i++)
			{
				if($strset[$i] == "1")
				{
					if($i >= 0 && $i <= 4)
					{
						if($newadd == 0) $this->filtercondition .= " OR ";
						$newadd = 0;
					}
					
					switch($i)
					{
					case 0: $this->filtercondition .= "type=".self::post_type_text; break;
					case 1: $this->filtercondition .= "type=".self::post_type_photo." OR type=".self::post_type_album." OR type=".self::post_type_albumphoto; break;
					case 2: $this->filtercondition .= "type=".self::post_type_link_url; break;
					case 3: $this->filtercondition .= "type=".self::post_type_link_audio; break;
					case 4: $this->filtercondition .= "type=".self::post_type_link_video; break;
					case 5: /* blog article */ break;
					default:
					}
				}
			}
	...
	function get_news_post_set($uid, $psince, $pmax, $psincemode, $psinceval)
	{
		$pmnumber = 5;
		$resposts = 0;
		$farray = array();
		...
		if(!$psinceval)
		{
			...
			$sqv2 = " WHERE visibility=1 $filter $searchcondition1 ORDER BY id DESC LIMIT $pmnumber"; 
			
		}else{
			if($psincemode == 0)
			{
				$sqv1 = "SELECT * FROM posts_";
				$sqv2 = " WHERE id<$psinceval AND visibility=1 $filter $searchcondition1 ORDER BY id DESC LIMIT $pmnumber";
				...
				$sqv1 = "SELECT * FROM posts_";
				$sqv2 = " WHERE wtime<'$tmval' AND visibility=1 $filter $searchcondition1 ORDER BY id DESC LIMIT $pmnumber";
			
			}else if($psincemode == 2){
			
				$sqv1 = "SELECT * FROM posts_";
				$sqv2 = " WHERE id>$psinceval AND visibility=1 $filter $searchcondition1 ORDER BY id DESC LIMIT $pmnumber";

				//$resposts = basics::query("SELECT * FROM posts_17 WHERE id<$psinceval ORDER BY id DESC LIMIT $pmnumber"); 
			}else if($psincemode == 3){
		...
		if($this->newspost_arraycount > $pmax)
		{
			$this->newspost_array = array_slice($this->newspost_array, 0, $pmax);
			$this->newspost_arraycount = $pmax;
		}
	 ...
	 *				2 - psince is time and in reverse mode (latest posts since)
	 *				3 - psince is time and in reverse mode (latest posts since)
	 */
	function get_posts_html_classic($uid, $psince, $pmnumber, $psincemode)
	{
		...
		if(($psincemode == 0 || $psincemode == 2) && $psince) /* find the position where the last post is */
		{
			$psva = explode("-", $psince);
		...
		}else if(($psincemode == 1 || $psincemode == 3) && $psince){ /* by time */
		...
		{		
			...
			if($llooptime == 0)$this->lptimeid = strtotime($rpost['wtime']);
	...
	/*
	 * mode 0 - default (classic)
	 * mode 1 - garden
	 * mode 2 - dark (for photo/video notes)
	 */
	...
	function get_feed_msg_html($text, $content, $puniqueid, $cuniqueid, $poaddmode, $org_puname, $org_wtime, $org_puslid, $org_puid)
	{
		$postm =	"<div class='feedc_poout$poaddmode' id='feedcpoout$puniqueid'><div class='feedc_poin$poaddmode'>
						<div class='feedc_potop'>
							<div class='feedc_potop_dp'><a href='#'><img src='data/u$org_puslid/dp/2.jpg' onerror='failsafe_img(this, 2);'/></a></div>
							<div class='feedc_potop_c'><div class='feedc_potitle'>
								<a href='#' class='feedc_pounamem'>$org_puname</a> $text</div>
	 ...
	 *  7 - just notes and header/dark (popup)
	 */
	...
	function get_singlepost_html($powner, $rpost, $feedmode, $pgenindex, $displaynote_count, $talkcls, &$pgenlast_markertime, &$puniqueid, $displaymode)
	{
		...
		if($rpost['type'] == self::post_type_blogarticle)
		{	
			return $this->get_feed_msg_html("added a new <a href='bu$pusid'>blog article</a>.", "", $puniqueid, $cuniqueid, $poaddmode, $org_puname, $org_wtime, $org_puslid, $pusid);
		}
		
		
		
		if($displaymode != 6 && $displaymode != 7)
		{
			$pcdata = $rpost['ctext'];
		...
		$shareobjid = "pidsobj".$puniqueid;
		
		
		$hpicdata = "";
		
		if($displaymode != 6 && $displaymode != 7) /* no content, just notes */
		{
			...
			if($cposttype == 3)
			{
				...
				if($mvrpdtext)
				{
					...
					{
						$hpicdata = "<div class='feedc_picb' id='$shareobjid'><div class='feedc_picm' >".
		...
		{			
			...
			for($i=0; $i<$nnotes; $i++)
			{
				...
				$postnotes .= 	"<div class='feedc_cm_line feedc".$displayadd."_cm_line_" . $oddev . "' id='feedc_cm_line_tct" . $noteccuid . "'><div class='feedc_cmli'>" .
								"<div class='feedc_cm_dp'><img src='data/u" . $noteculid . "/dp/3.jpg'/></div>" .
								"<div class='feedc".$displayadd."_cm_c'>$notedelp" .
								"<div id='feedc_cm_line_tdt" . $noteccuid . "'><a onmouseover='vusr(this, \"$notepusid\");' href='u$notepusid' class='feedc_pouname'>" . $notecuname . " </a>" . $inote .
									"</div><div class='feedc_ponote_ctr'><abbr class='synctime' id='" . $notetsyncid . "' data-mode='0' data-ts='" . $inotetime . "'></abbr> <div style='display: inline-block;' id='mkcd$puniqueid-$noteccuid'>$notemarkshtml</div> <samp> . <a data-attch='mkcd$puniqueid-$noteccuid' data-nid='$puniqueid"."-$noteccuid' data-ua='".$notemarkvalsuc[0]."' data-ub='".$notemarkvalsuc[1]."' data-uc='".$notemarkvalsuc[2]."' data-ud='".$notemarkvalsuc[3]."' data-ue='".$notemarkvalsuc[4]."' data-a='".$notemarkvalsac[0]."' data-b='".$notemarkvalsac[1]."' data-c='".$notemarkvalsac[2]."' data-d='".$notemarkvalsac[3]."' data-e='".$notemarkvalsac[4]."' onclick='markbox_show(this);'>Mark</a></samp></div>" .
								"</div><div style='clear: both;'></div></div></div>";
			}
		...
		$ncnotediv = "";
		$feedpopup_mode = 1; /* own post */
		
		if($cuid != $puid)
		{
			if($feedmode == 0) $feedpopup_mode = 2; /* somebody else's on user's feed */
			else $feedpopup_mode = 3; /* none of our business */
		}
		...
		if($repost_fromuser)
			$putousername .= " <a style='color: #afaaaaa;'> from </a> <a href='pu".basics::quick_uida($repost_fromuser)."' class='feedc_pouname' style='font-weight: normal;'>".basics::get_username_from_id($repost_fromuser)."</a>";
		...
		if($displaymode == 0)
		{
			$postm =	"<div class='feedc_poout$poaddmode' id='feedcpoout$puniqueid'>$posttmarker<div class='feedc_poin$poaddmode'>".
							"<span><div class='feedc_boxc$poaddmode'>$ncnotediv<span>$icmsourcebtn<div title='Share' class='boxc_i boxc_share' onclick=\"repost_publish(0, '$popuid', '$poppid', '$shareobjid');\" style='margin-right: 2px;' ></div><div class='boxc_i boxc_close' onclick='show_feedpopup($feedpopup_mode, this, 1, \"$puniqueid\");'></div></span></div></span>".
							"<div class='feedc_potop'>".
								"<div class='feedc_potop_dp'><a href='$linkuid_pusid'><img src='data/u$org_puslid/dp/2.jpg' onerror='failsafe_img(this, 2);'/></a></div>".
								"<div class='feedc_potop_c'><div class='feedc_potitle'>".
									"<a href='$linkuid_pusid' class='feedc_pounamem' $linkuid_vusr>$org_puname</a>$putousername</div>";
		}else{
			...
			$postm =	"<div class='feedc_psoout$poaddmode' id='feedcpoout$puniqueid'>$posttmarker<div class='feedc_psoin$poaddmode'>".
							"<span><div class='feedc_boxc$poaddmode'>$ncnotediv<span><div title='Share' class='boxc_i boxc_share' onclick=\"repost_publish(0, '$popuid', '$poppid', '$shareobjid');\" style='margin-right: 2px;' ></div><div class='boxc_i boxc_close' onclick='show_feedpopup(1, this, 1, \"$puniqueid\");'></div></span></div></span>".
		...
		if($rpost['loc_used'])
		{
			$location_html = "<i class='feedc_post_loctag' onclick=\"show_mapcard(this, 1, '".$rpost['loc_lt'].",".$rpost['loc_lg']."');\" >".$rpost['loc_name']."</i> . ";
		}
		
		if($displaymode == 7) /* no buttons for share and delete */
		{
			$cbarvals =	" . <a href=\"javascript:repost_publish(0, '$popuid', '$poppid', '$shareobjid');\">Share</a> . Edit . <a onclick='show_feedpopup(1, this, 1, \"$puniqueid\");'>Menu</a>" . $cbarvals;
			$postm = "<div class='feedc_psoout$poaddmode' id='feedcpoout$puniqueid'>$posttmarker<div class='feedc_psoin$poaddmode'>".
					  "<div class='feedc_psotop'><div class='feedc_psotop_c'>";
		}
		
		$notebuttonfunction = "fc_comment_button_ex";
		
		if($displaymode == 7)
			$notebuttonfunction = "fullphoto_notebutton";
		...
		$postm .=	"<div class='$feedc_potop_t'><div id='pidtds$puniqueid'>$pcdatam</div><div id='pidtdl$puniqueid' style='display: none;'>$pcdata</div></div>".
								$hpicdata.
								"<div class='feedc".$displayadd."_potop_ctr'>".
									"<a data-attch='mkcd$puniqueid' data-nid='$puniqueid"."_0' data-ua='".$ucmarkarray[0]."' data-ub='".$ucmarkarray[1]."' data-uc='".$ucmarkarray[2]."' data-ud='".$ucmarkarray[3]."' data-ue='".$ucmarkarray[4]."' data-a='".$cmarkarray[0]."' data-b='".$cmarkarray[1]."' data-c='".$cmarkarray[2]."' data-d='".$cmarkarray[3]."' data-e='".$cmarkarray[4]."' onclick='markbox_show(this);'>Mark</a> . <a href='javascript: $notebuttonfunction(\"pcd$puniqueid\");'>Note </a>$cbarvals <div style='display: inline-block;' id='mkcd$puniqueid'>$cmarkvals</div> . ".
									"$location_html<a href='c$cuniqueid'><abbr class='synctime' data-ts='$org_wtime' data-mode='0'></abbr></a>".
	...
	function delete_postex($psuid, $psvad)
	{
		...
		if($psuid == $cuid)
		{
			...
			$resposts = basics::query("SELECT * FROM posts_$psuid WHERE id=$psvad"); /* can delete any post since it's the user's profile */
		}
		...
		/* if the post is associated with a photo/album, do not delete the post but just hide it */
		
		if($rpost['type'] == self::post_type_album || $rpost['type'] == self::post_type_albumphoto)
		{
			if($psuid == $cuid)
			{
				$resposts = basics::query("UPDATE posts_$psuid SET visibility=0 WHERE id=$psvad");
			}else{ /* delete the whole photo/album if it's on somebody else's profile */
				/* [todo] */
			}
			return 1;
		}
	...
	function get_lptimeid()
	{
		return $this->lptimeid;
	}
	...
	function get_singlepost_html_garden($powner, $rpost, $feedmode, $pgenindex, $displaynote_count, $talkcls, &$pgenlast_markertime, &$puniqueid, $displaymode)
	{
		...
		if($displaymode != 6) /* no content, just notes */
		{
			...
			if($cposttype == 3)
			{
				...
				if($mvrpdtext)
				{
					...
					if($displaymode == 0) /* normal post, thumbnail photo */
					{
						$hpicdata = "<div class='feedc_picb'><div class='feedc_picm' >".
									"<img data-src='$turl'/ onload='newpostimg_center(this);' onerror='newpostimg_err(this)'/>".
								"<div class='feedc_picm_vc' onclick=\"fc_post_expand_video('$mvsite', '$mvurl', 'pidtdlvttl$puniqueid', 'pidtdl$puniqueid', '$puname', '$puslid', '$pusid', '$puniqueid')\"><div class='feedc_picm_vcplay'></div><div class='feedc_picm_vcdur'>$mvdur</div></div></div><a href='$mvurl' style='text-decoration: none;' target='_blank'><div class='feedc_picbvt' id='pidtdlvttl$puniqueid'>$mvtitle</div></a><div class='feedc_picbvs'>$mvsite</div></div>";
				...
				$hpicdata .= "<div class='feedc_picb'><div class='feedc_picm' >".
									"<img data-src='$postphotoctfile'/ onload='newpostimg_center(this);' onclick=\"fc_post_expand_album('$pusid', $postphotoctalbumid, $postphotoctcvpid, '$puniqueid')\">".
								"</div><!--<a href='albums.$pusid-$postphotoct_albumsid' style='text-decoration: none;'><div class='feedc_picbvt' id='pidtdlvttl$puniqueid'>$postphotoct_albname</div></a><div class='feedc_picbvs'></div><div class='feedc_picbvl'>$postphotoct_albloc</div>--> 	</div>";
								"<div class='singlepost_ddetails'>".
										"<div class='singlepost_ddetails_t'></div>".
										"<div class='singlepost_ddetails_u'></div>".
		...
		$postm = "<div class='feedg_postbox'>
					<div class='feedg_postbox_basics'>
						...
						<div class='feedg_postbox_basicsdp'>
							<img src='data/u$org_puslid/dp/1.jpg' onerror='failsafe_img(this, 1);'/>
						</div>
						<div>$hpicdata</div>
						<div class='feedg_postbox_mc'>
						
						<div>
						<div class='feedg_postbox_basicstitle'>$org_puname</div>
						</div>
						...
						<div class='feedg_post_write' onclick='fc_comment_button_ex(\"pcd$puniqueid\")' style='outline: none;' id='pcd" . $cuniqueid . "acit' contenteditable='plaintext-only'>Write</div>
