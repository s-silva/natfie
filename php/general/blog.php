...
class blog
{
	...
	function articles_get($uid, $psince, $pmnumber, $psincemode, $psinceval)
	{
		$resposts = 0;
	...
	function articles_get_next(&$postowner)
	{
	 ...
	 */
	function get_blog_html($uid, $psince, $pmnumber, $psincemode)
	{
		...
		$resposts = $this->articles_get($uid, $psince, $pmnumber, $psincemode, $psinceval);
		...
		while($rpost = $this->articles_get_next($postowner))
		{		
			$puid    = $rpost['uid'];
			$pgenindex++;
			...
			$postm = $this->article_get_html($postowner, $rpost, $feedmode, $pgenindex, $displaynote_count, $talkcls, $pgenlast_markertime, $puniqueid, 0);
	...
	function article_get_html($powner, $rpost, $feedmode, $pgenindex, $displaynote_count, $talkcls, &$pgenlast_markertime, &$puniqueid, $displaymode)
	{
		...
		if($rpost['type'] != self::post_type_blogarticle) return "";
		...
		if($displaymode != 6)
		{
			...
			$pcdata = str_replace("[-n-l-]", "<br/>", $pcdata);
			
			$pcdata = basics::decode_tagcodes($pcdata);
			$pcdata = basics::translate_urls_to_html($pcdata);
			$pcdatam = self::trim_text($pcdata, 3000, true, "... <div class='feedc_potop_exp'><a href='javascript: fc_post_expand(\"pidtds$puniqueid\", \"pidtdl$puniqueid\", 0)'>...</a></div>");
		...
		$article_title = "";
		$media_title = 0;    /* title on the footer, album name for photos... */
		...
		if($displaymode != 6) /* no content, just notes */
		{
			...
			if($cposttype == 1 && $postmode == 1)
			{
				...
				$hpicdata = "<div class='blogpostreblogtext blogposttext'><div class='feedc_picm_dtquote'><div id='sp_pidtds$puniqueid'>$sp_pcdatam</div><div id='sp_pidtdl$puniqueid' style='display: none;'>$sp_pcdata</div></div></div>";
			}
			...
			if($cposttype == 3)
			{
				...
				if($mvrpdtext)
				{
					...
					$media_title = "<a href='$mvurl'>$mvtitle</a>";
					...
					if($displaymode == 0) /* normal post, thumbnail photo */
					{
						...
						*/
						...
						$hpicdata = "<center><div class='blogpostphoto_block'>".
										"<img data-src='$turl'/ width='903px' onload='blog_photo_center(this);' onerror='newpostimg_err(this)' onclick=\"fc_post_expand_video('$mvsite', '$mvurl', 'pidtdlvttl$puniqueid', 'pidtdl$puniqueid', '$puname', '$puslid', '$pusid', '$puniqueid')\"/>".
									"</div></center>";
				...
				$postphotoctcvpid = 0;
				$postphotoctfile = $postphotoct->photo_getfile($powner, $rpost['div1'], 0, 2, $postphotoctcvpid);
				...
				$media_title = $postphotoct_albname;
				
				$hpicdata .=	"<center><div class='blogpostphoto_block'>".
									"<img data-src='$postphotoctfile'/ width='903px' onload='blog_photo_center(this);' onclick=\"fc_post_expand_album('$pusid', $postphotoctalbumid, $postphotoctcvpid, '$puniqueid')\"/>".
								"</div></center>";
								
								/*	<a href='albums.$pusid.$postphotoct_albumsid' style='text-decoration: none;'><div class='feedc_picbvt' id='pidtdlvttl$puniqueid'>$postphotoct_albname</div></a><div class='feedc_picbvs'></div><div class='feedc_picbvl'>$postphotoct_albloc</div></div>";
		...
		$sourceusername = "<a class='blogpinfopi_user' href='$linkuid_pusid' $linkuid_vusr>$org_puname</a>";
		
		if($repost_fromuser)
			$sourceusername = "<a class='blogpinfopi_user' href='pu".basics::quick_uida($repost_fromuser)."'>".basics::get_username_from_id($repost_fromuser)."</a>";
		...
		*/
		...
		if($pcdatam != "")
		{
			$btitle_spos = strpos($pcdatam, ":endof-natfie-title:");
			
			if($btitle_spos !== false)
			{
				$article_title = substr($pcdatam, 0, $btitle_spos);
				$pcdatam = substr($pcdatam, $btitle_spos + strlen(":endof-natfie-title:"));
			}
		}
		
		$postm = "<div class='blogpostouter'><div class='blogpostout'><div class='blogpost' onmouseover=\"javascript: fadein_item('blogpanels_panel_$puniqueid', 1);\">
				<div class='blogposttpanel' id='blogpanels_panel_$puniqueid'>
					<a href=\"javascript:repost_publish(0, '$popuid', '$poppid');\"><div class='blogtopbutton'><div style='padding: 17px 17px;'><div class='blogbtnmttag' title='Tag this post'></div></div></div></a>
					<a href='#'><div class='blogtopbutton'><div style='padding: 19px 14px;'><div class='blogbtnmtsource' title='Go to the original post'></div></div></div></a>
					<a href=\"javascript:repost_publish(0, '$popuid', '$poppid');\"><div class='blogtopbutton'><div style='padding: 19px 13px;'><div class='blogbtnmtreblog' title='Reblog this'></div></div></div></a>
				</div>
				<div style='clear:both;'></div>

				<div class='blogposttitle'>$article_title</div>
				...
				<div class='blogpostcontent'>
					...
					$hpicdata
					<div class='blogpinfopct_text'>
					
						<p>".($media_title ? "<span class='blogpinfopi_title'>$media_title</span> | " : "")."$sourceusername
						</p><p><span class='blogpinfopi_loc'>Viviens, France<span></p>
						<p><span class='blogpinfopi_time'><abbr class='synctime' data-ts='$org_wtime' data-mode='1'></abbr><span>".($ncnotes ? " | <a href='#' class='blogpinfopi_notes'>$ncnotes Notes</a>" : "")."</p>
				...
				/*
				<div class='blogpostcontent'>
					<div class='blogpostphoto'>
						<figure>
							<img data-src='images/test/photo1.jpg'/>			
							<div class='blogpinfopct_photo'>
					
								<p><span class='blogpinfopi_title'>Sidewalk to Paradise</span> | <a href='#' class='blogpinfopi_user'>Ryan Martini</a>
								<p><span class='blogpinfopi_loc'>Viviens, France<span></p>
								<p><span class='blogpinfopi_time'><abbr>April 21st, 2012</abbr><span> | <a href='#' class='blogpinfopi_notes'>11 Notes</a></p>
	...
	function write_theme_data($uid, $data)
	{
		$uname = basics::get_username_from_id($uid);
		
		$data = str_replace("<blog.title>", $uname."'s Blog", $data);
		$data = str_replace("<blog.dsc>", "I could not help laughing at the ease with which he explained his process of deduction. \"When I hear you give your reasons,\" I remarked, \"the thing always appears to me to be so ridiculously simple that I could easily do it myself", $data);
